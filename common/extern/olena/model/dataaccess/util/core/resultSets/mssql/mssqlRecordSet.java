...
public class mssqlRecordSet extends recordSet_Model{

	public mssqlRecordSet(IItem_Model itemInfos) {
		super(itemInfos);
	}
	@Override
	public boolean PrepareStatementInsert(String strSql) throws SQLException {
		...
		try {
			...
			m_statement = m_objConnection.prepareStatement(strSql, PreparedStatement.RETURN_GENERATED_KEYS);
	...
	public Object executePrepareStatementInsert() throws SQLException {
		...
		try {
			...
			m_resultSet = ((PreparedStatement)m_statement).getGeneratedKeys();
			if(m_resultSet.next()) {
				result  = m_resultSet.getObject(1);
			}
		}
		catch (SQLException ex) { throw ex; }
		return result;
	...
	public Object executeStatementInsert(String strSql) throws SQLException {
		...
		try {
			...
			m_statement.executeUpdate(strSql, Statement.RETURN_GENERATED_KEYS);
	...
	public Vector<Object> productGetFirstInfoList(pageExpressionRelation pageInfo) throws Throwable {
		...
		try {
			if(nPageCount > 0) {	//Page count determined
				if(nPageSize > 0) {	//Page size exist
					setPageSize(pageInfo.getPageSize());
					if(nPageIndex < 0) { pageInfo.setPageIndex(0); }
					AbsolutePage(pageInfo.getPageIndex());
				...
				if(nPageSize > 0) {	//Page size determined
